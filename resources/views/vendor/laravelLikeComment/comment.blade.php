...
<div class="laravelComment" id="laravelComment-{{ $comment_item_id }}">
    ...
    <div class="ui threaded comments" id="{{ $comment_item_id }}-comment-0">
        <button class="btn btn-primary btn-md submit button" id="write-comment" data-form="#{{ $comment_item_id }}-comment-form">Write comment</button>
        <form class="ui laravelComment-form form" id="{{ $comment_item_id }}-comment-form" data-parent="0" data-item="{{ $comment_item_id }}" style="display: none;">
            <div class="field md-form">
                <textarea class="form-control md-textarea ml-2" id="0-textarea" rows="2" {{ $GLOBALS['commentDisabled'] }}></textarea>
                @if(!Auth::check())
                    <small>Please Log in to comment</small>
                @endif
            </div>
           <button type="submit" class="btn btn-primary btn-sm submit button" {{ $GLOBALS['commentDisabled'] }}>Comment</button>
        </form>
<?php
    ...
    <div class="comment show-{{ $comment->item_id }}-{{ (int)($GLOBALS['commentClass'] / 5) }}" id="comment-{{ $comment->id }}">
        ...
        <div class="content">
            ...
            <form id="{{ $comment->id }}-reply-form" class="ui laravelComment-form form" data-parent="{{ $comment->id }}" data-item="{{ $comment->item_id }}" style="display: none;">
                <div class="field md-form">
                    <textarea class="form-control md-textarea" id="{{ $comment->id }}-textarea" rows="2" {{ $GLOBALS['commentDisabled'] }}></textarea>
                    @if(!Auth::check())
                        <small>Please Log in to comment</small>
                    @endif
                </div>
...
?>
    ...
    @if(count($comments) > 0)
    <button class="btn btn-primary btn-xs button" id="showComment" data-show-comment="0" data-item-id="{{ $comment_item_id }}">Show comments</button>
    @endif
</div>
